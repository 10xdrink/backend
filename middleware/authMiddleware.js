  ...
  try {
    ...
    try {
      ...
      if (!user.lastActivity) {
        ...
        await user.save();
        logger.info(`Initialized lastActivity for user: ${user._id}`);
      }
      ...
      // Log the session error but allow the request to continue
      logger.error(`Session service error: ${sessionError.message}`, { userId: user._id });
      // Continue with authentication without session management
    }
