...
const { sendEmail } = require('../services/emailService'); // Correctly destructure the sendEmail function
const Joi = require('joi'); // **Added Import for Joi**
const currencyUtils = require('../utils/currencyUtils'); // Update imports to include currency utilities
...
exports.createOrder = asyncHandler(async (req, res, next) => {
  const { items, shippingAddress, billingAddress, paymentMethod, couponCode, phone } = req.body;
  ...
  // Create order items and calculate USD total
  const orderItems = [];
  let totalUSD = 0;
  ...
  // Convert USD total to INR
  let totalINR = totalUSD * USD_TO_INR_RATE;
  let discountUSD = 0;
  let discountINR = 0;
  ...
  if (couponCode) {
    ...
    const couponValidation = await Coupon.canApplyCoupon(coupon);
    ...
    discountUSD = coupon.discountType === 'percentage'
      ? (totalUSD * (coupon.discount / 100))
      : coupon.discount;

    // Convert discount to INR
    discountINR = discountUSD * USD_TO_INR_RATE;

    // Apply discount to total
    finalAmount = totalINR - discountINR;
  ...
  const order = await Order.create({
    ...
    billingAddress,
    status: 'pending',
    paymentStatus: ['cod', 'cash_on_delivery'].includes(paymentMethod.toLowerCase()) ? 'pending' : 'paid',
  ...
  try {
    for (const item of items) {
      ...
      if (variantIndex !== -1) {
        ...
        // Log stock reduction
        logger.info(`Stock reduced for product ${product.title}, variant ${item.variant} by ${item.quantity} units. New stock: ${product.variants[variantIndex].stock}`);
    ...
    // Note: We don't want to fail the order creation if stock reduction fails
  ...
  try {
    ...
    <body>
      <div class="container">
        <div class="logo">
          <img src="https://res.cloudinary.com/dvbbsgj1u/image/upload/v1731090126/tesnhlpvlo6w9bdjepag.png" alt="10X Logo">
        </div>
        
        <h2>Order Confirmation</h2>
        <p>Hi ${req.user.name},</p>
        <p>Thank you for your purchase! Your order has been received and is currently pending approval.</p>
        ...
        <div class="order-details">
          ...
          <div class="price-breakdown">
            ...
            <div class="price-item">
              ...
              <span>$${order.totalAmountUSD.toFixed(2)}</span>
            </div>
            <div class="price-item">
              <span>Conversion Rate:</span>
              <span>₹${USD_TO_INR_RATE} per USD</span>
            </div>
            <div class="price-item">
              <span>Product Total (INR):</span>
            ...
            <div class="price-item discount">
              ...
              <span>-₹${order.discountINR.toFixed(2)}</span>
            </div>
            <div class="price-item">
              <span>Amount after Discount:</span>
              <span>₹${(order.totalAmountINR - order.discountINR).toFixed(2)}</span>
            </div>
            ` : ''}
            <div class="price-item">
        ...
        <p>If you have any questions or need assistance, please <a href="${process.env.SUPPORT_URL || 'http://localhost:5173/contact'}">contact our support team</a>.</p>
...
exports.getAllOrders = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Process orders to include final amount
    const enrichedOrders = orders.map(order => {
      const orderObj = order.toObject({ virtuals: true });
      
      // Calculate total from items and convert to INR
      const itemsTotal = order.items.reduce((sum, item) => {
        // Convert each item's price from USD to INR
        const priceInINR = Number(item.price) * USD_TO_INR_RATE;
        return sum + (priceInINR * Number(item.quantity));
      }, 0);
      
      // Set the total amount in INR
      orderObj.totalAmount = itemsTotal;
...
exports.getMyOrders = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Make sure we include virtual fields like finalAmount in the response
    const enrichedOrders = orders.map(order => {
      const orderObj = order.toObject({ virtuals: true });
      // Ensure finalAmount is calculated correctly
      if (orderObj.finalAmount === undefined) {
        orderObj.finalAmount = orderObj.totalAmount - (orderObj.discount || 0) + (orderObj.shippingFee || 0);
      }
      return orderObj;
    });
...
exports.getOrderById = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role === 'user' && order.customer.toString() !== req.user._id.toString()) {
    return res.status(403).json({
      success: false,
      message: MESSAGES.GENERAL.UNAUTHORIZED || 'You are not authorized to access this order.',
    });
  }
  
  // Make sure we include virtual fields and calculate final amount
  const orderObj = order.toObject({ virtuals: true });
  
  // Calculate final amount after discount and shipping
  orderObj.finalAmount = order.finalAmount || (order.totalAmountINR - (order.discountINR || 0) + (order.shippingFee || 0));
...
const permanentlyDeleteOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await Refund.deleteMany({ orderId: id }, { session });

    // Delete related return requests if any
    await ReturnRequest.deleteMany({ orderId: id }, { session });
    ...
    res.json({
      success: true,
      message: 'Order and related records permanently deleted',
    });
  } catch (error) {
...
const getOrderStats = asyncHandler(async (req, res) => {
  try {
    // Get stats from the last 30 days by default or from query params
    ...
    // Ensure the dateTo is the end of the day
    ...
    const returnedOrders = await Order.countDocuments({ ...dateFilter, status: 'returned' });
    const refundedOrders = await Order.countDocuments({ ...dateFilter, status: 'refunded' });
    ...
    const averageOrderValue = revenueAggregate.length > 0 && revenueAggregate[0].countOrders > 0 
      ? Math.round(revenueAggregate[0].totalRevenue / revenueAggregate[0].countOrders) 
      : 0;
    ...
    // Get daily orders count for the period
    const dailyOrdersAggregate = await Order.aggregate([
      { $match: dateFilter },
      { $group: {
    ...
    const dailyOrders = dailyOrdersAggregate.map(day => ({
      date: day._id,
      count: day.count,
      revenue: day.revenue
    }));
...
module.exports = {
  ...
  refundOrder: exports.refundOrder,
  bulkUpdateOrders: exports.bulkUpdateOrders,
  getOrderMetrics: exports.getOrderMetrics,
  getMetricsCache: exports.getMetricsCache,
